...
// enumSerialPorts uses SetupAPI to enumerate present Ports-class devices.
...
func openSerial(portName string) (windows.Handle, error) {
	...
	// Match pySerial's Windows configuration: binary mode, RTS/CTS handshaking,
	// and DTR enabled.
...
func renderTextLinesBox(width, height int, lines []string) *image.RGBA {
	canvas := image.NewRGBA(image.Rect(0, 0, width, height))
	draw.Draw(canvas, canvas.Bounds(), &image.Uniform{color.White}, image.Point{}, draw.Src)
	...
	startY := (height-totalHeight)/2 + metrics.Ascent.Ceil()
	...
	for i, line := range lines {
		...
		d.Dot = fixed.P(4, startY+i*lineHeight)
		d.DrawString(line)
	}
...
func sendDisplayBitmapRevA(handle windows.Handle, x0, y0, x1, y1 int, img image.Image) error {
	...
	// The upstream Python implementation chunks by the display width, not the
	// cropped bitmap width. The device appears to expect that framing.
	chunkSize := 320 * 8
...
func readPrimaryAdapterDedicatedVideoMemory() (uint64, error) {
	...
	log.Printf("[gpu] adapter=0 name=%q vendor=0x%04X device=0x%04X dedicated=%s shared=%s", adapterName, desc.VendorID, desc.DeviceID, formatBytesGiB(desc.DedicatedVideoMemory), formatBytesGiB(desc.SharedSystemMemory))
...
func main() {
	...
	updateStats := func() error {
		...
		log.Printf("stats update: line1=%q (%d chars), line2=%q (%d chars), line3=%q (%d chars), line4=%q (%d chars)",
			line1, len(line1), line2, len(line2), line3, len(line3), line4, len(line4))
		log.Printf("stats bitmap: VRAM %dx%d, util %dx%d, RAM %dx%d, CPU %dx%d", statsW, statsH, utilW, utilH, ramW, ramH, cpuW, cpuH)
		log.Printf("stats payload: VRAM %d bytes, util %d bytes, RAM %d bytes, CPU %d bytes",
			statsW*statsH*2, utilW*utilH*2, ramW*ramH*2, cpuW*cpuH*2)
		...
		return sendDisplayBitmapRevA(handle, cpuX, cpuY, cpuX+cpuW-1, cpuY+cpuH-1, renderTextBox(cpuW, cpuH, line4))
	}
